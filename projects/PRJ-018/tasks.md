...
| **AS-CLEAN-18** | **owner smoke fail 時の diagnostic bundle 自動収集** — QW3/QW5/QW6 trauma (実機固有 issue が手元で再現困難) からの教訓を制度化。`scripts/asagi-diagnostic-bundle.ps1` で `~/.asagi/` 配下 + DB SQLite header parse + `~/.codex/auth.json` metadata + Codex CLI version + lint 結果 + OS info + `ASAGI_*` env を収集 → `~/.asagi/diagnostic-bundle-yyyyMMdd-HHmmss.zip` 出力。PII redaction 規約: history.db は header 256 byte のみ / store.json は keys のみ / auth.json は metadata のみ / keyring access せず。owner が smoke fail 時 1 発実行 → CEO/Dev へ zip 共有で 「ファイル何バイト?」「PRAGMA 何返した?」の Q&A 往復ゼロ化、解析時間 30 分 → 5 分目標 | CEO 直接 | 1.0h | — | [x] **2026-05-05 完了** (commit `a6c638f`、`scripts/asagi-diagnostic-bundle.ps1` 新規 1 ファイル ~270 行、DB header 自動診断 (OK / STALE / ABNORMAL) 実装、CEO 自身の環境で smoke 実施 → STALE DB を `STALE: empty SQLite file (schema 0, encoding 0) -- AS-HOTFIX-QW5 reproducer pattern` と一発判定 PASS、handoff doc § 6.1 に手順追記、ASCII safe (mojibake 0)) |
